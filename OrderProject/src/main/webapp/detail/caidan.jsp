		...
		<div id="menu">
				...
				<ul>
					...
					<c:forEach items="${list}" var="c">
						<li>
							<dl>
								<dt>
									<a href="caixiangxi.web?id=${c.id}&tableId=${tableId}"> <img alt=""
										src="upload/${c.img}" width="214px" height="145px" />
									</a>
								</dt>
								<dd class="f1">
									<a href="caixiangxi.web?id=${c.id}&tableId=${tableId}">${c.foodname }</a>
								</dd>
								<dd class="f2">
									<a href="caixiangxi.web?id=${c.id}&tableId=${tableId}">${c.price}</a>
								</dd>
							</dl>
						</li>
					</c:forEach>
			...
			<div id="foot">
				...
				<div id="pages" style="position: relative; top: 15px">
					...
					<c:if test="${page!=1 }">
						<a href="caidan.web?page=${page-1 }&tableId=${tableId}">上一页</a>
					</c:if>
					<c:forEach begin="1" end="8" var="i">
						<!-- 若循环到了当前页，则将页码高亮显示 -->
						<c:if test="${i==page }">
							<a href="caidan.web?page=${i}&tableId=${tableId}" class="current_page">${i }</a>
						</c:if>
						<!-- 若不是当前页，则去掉高亮的样式 -->
						<c:if test="${i!=page }">
							<a href="caidan.web?page=${i}&tableId=${tableId}">${i }</a>
						</c:if>
					</c:forEach>
					<c:if test="${page==8 }">
						<a href="">下一页</a>
					...
					<c:if test="${page!=total }">
						<a href="caidan.web?page=${page+1 }&tableId=${tableId}">下一页</a>
					</c:if>
